...
class SvgCreatePage extends StatefulWidget {
  @override
  _ImageMarkerPageState createState() => _ImageMarkerPageState();
}

class _ImageMarkerPageState extends State<SvgCreatePage> {
  ui.Image? image;
  List<Marker> markers = [];
  ...
  Future<void> _saveAsSvg() async {
    ...
    try {
      await DatabaseHelper.instance.insertSvg(svgString);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Image saved as SVG and to database')),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
